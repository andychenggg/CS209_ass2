    ...
    while (!ClientApplication.Over) {
      ...
      try {
        ...
        switch (hugeMessage.getProtocol()) {
          ...
          case NEW_ONLINE_CLIENT: {
            if (hugeMessage instanceof NewClientOnlineMessage) {
              ...
              ClientApplication.updateClientList(
                  ((NewClientOnlineMessage) hugeMessage).getUserClient().values());
            }
            break;
          }
          case OFFLINE: {
          ...
          case FILE: {
            if (hugeMessage instanceof Message) {
              System.out.println(
                  "Receive hm in Client ThreadIn" + ClientApplication.getController());
              System.out.println(((Message) hugeMessage).getSendTo());
              ClientApplication.getController().addRecent(((Message) hugeMessage).getSentBy());
              ClientApplication.getController()
                  .putClientAhead(Collections.singletonList(((Message) hugeMessage).getSentBy()));
          ...
          case CREATE_GROUP: {
            if (hugeMessage instanceof CreateGroupMessage) {
              ...
              ClientApplication.getController()
                  .showChatStage(((CreateGroupMessage) hugeMessage).getClients());
            }
            break;
          }
          case QUIT_GROUP: {
            if (hugeMessage instanceof QuitMessage) {
              ...
              ClientApplication.getController().getChatCTR(q.getCurrentGroups())
                  .removeAvatar(q.getUserName());
              // give a warning
              ClientApplication.getController()
                  .showWarningInController("User " + q.getUserName() + " has been offline. ");
            }
          }
        }
        ...
        try {
          ...
          Platform.runLater(() -> {
            ClientApplication.getController().showWrongMessageInController(
                "Server has been shut down for unknown reason!\n Application will be closed in 3 seconds.");
            new Thread(() -> {
              try {
                Thread.sleep(3000);
